...
describe("extract-data", function() {
  ...
  describe("flow", function() {
    ...
    xit("should use the diff rather than checking out full revision", function(done) {
    ...
    it("should checkout the files at the revision rather than using diffs", function(done) {
      var repoStatsOverTime = {
        branchName: "experiment/improving-visibility_measurements",
        resultsJsonDirname: "results",
        startingRevision: "7ce9439c132ab7c76d62750831cbdff5f1e1bbf2",
      ...
      pipeline.getRevisionsList(repoStatsOverTime)
        .then(pipeline.getFileContentsAtRevisions, generalPurposeCatchFunction)
        .then(pipeline.exportAsTable, generalPurposeCatchFunction)
        .then(function(result) {
          expect(result).toBe(repoStatsOverTime);
